-- ✅ SOLUTION FINALE: Le problème vient de hnsw.ef_search = 40 (par défaut)
-- Ce paramètre limite le nombre de résultats retournés par l'index HNSW
-- Solution: Augmenter hnsw.ef_search à 1000 (maximum) dans la fonction RPC

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS public.search_albums CASCADE;

-- Recréer avec SET LOCAL hnsw.ef_search
...
AS $$
BEGIN
  -- ⭐ CETTE LIGNE RÉSOUT LE PROBLÈME DES 40 RÉSULTATS ⭐
  -- Augmenter ef_search au maximum (1000) pour permettre jusqu'à 1000 résultats
  -- Par défaut, ef_search = 40, ce qui limitait nos recherches !
  EXECUTE 'SET LOCAL hnsw.ef_search = 1000';
